...
def weights_download(out='F:/PROJECT/od/odApp/models/yolov3.weights'):
    _ = wget.download(
        'https://pjreddie.com/media/files/yolov3.weights', out='F:/PROJECT/od/odApp/models/yolov3.weights')
...
def main():
    ...
    load_darknet_weights(
        yolo, 'F:/PROJECT/od/odApp/models/yolov3.weights')
    ...
    while (True):
        ...
        class_names = [c.strip() for c in open(
            "F:/PROJECT/od/odApp/models/classes.TXT").readlines()]
        ...
        xcor1=x
        xcor2=x+w
       # xcor_center=int(M['m10']/M['m00'])

        #print("largerBox")
        #print("x cord:",str(xcor1))
        #print("x cord2",str(xcor2))
        #print("")

        ycord1=y
        ycord2=y+h
        #ycord_center=int(M['m01']/M['m00'])

        #print("ycord1",str(ycord1))
        #print("ycord2",str(ycord2))
        xval1=str(xcor1)
        xval2=str(xcor2)
        yval1=str(ycord1)
        yval2=str(ycord2)

        if xcor1>=0 and xcor2<=300:
                direct='left'
        elif xcor1>=300 and xcor2<=450:
                direct='middle'
        else:
            direct='right'            
        direction=str(xcor2)
        #cv2.putText(image, direct, (20,40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 3)
        image, val = draw_outputs(image, (boxes, scores, classes, nums), class_names)
